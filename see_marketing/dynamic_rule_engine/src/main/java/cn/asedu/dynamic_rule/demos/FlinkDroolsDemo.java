    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<String> result = connectedStream.process(new KeyedBroadcastProcessFunction<String, Applicant, String, String>() {
            ...
            public void processBroadcastElement(String value, Context ctx, Collector<String> out) throws Exception {
                ...
                // value:  rule-name,rule-code
                String[] split = value.split(",");

                KieHelper kieHelper = new KieHelper();
                KieSession kieSession = kieHelper.addContent(split[1], ResourceType.DRL).build().newKieSession();


                log.warn("收到一条新的规则，并插入了state");
                broadcastState.put(split[0],kieSession);
